<section class="min-h-screen flex items-center justify-center py-12 px-6">
    <div class="bg-white shadow-2xl rounded-2xl w-full max-w-3xl p-10 border border-blue-200">
        ...
        <form method="POST" action="{{ route('form.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <!-- Upload Bukti -->
            <div>
                <label class="block text-sm font-semibold text-blue-800 mb-2">Bukti Kejadian (Pilih salah satu)</label>

                <div class="space-y-3">
                    <div>
                        <label class="block text-blue-700 mb-1">Gambar</label>
                    ...
                    <div>
                        <label class="block text-blue-700 mb-1">Video</label>
                        <input type="file" name="video" id="video" accept="video/*"
                            class="w-full px-4 py-2 border border-blue-300 rounded-xl bg-blue-50 file:bg-transparent file:border-none focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>

                    <div>
                        <label class="block text-blue-700 mb-1">Audio</label>
                        <input type="file" name="audio" id="audio" accept="audio/*"
                            class="w-full px-4 py-2 border border-blue-300 rounded-xl bg-blue-50 file:bg-transparent file:border-none focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>

                    <p class="text-xs text-blue-600 mt-1">Silakan pilih <strong>salah satu</strong> bukti (gambar, video, atau audio).</p>
                </div>
            </div>
    ...
    <script>
        const inputs = ['gambar', 'video', 'audio'];
        ...
        inputs.forEach(id => {
            document.getElementById(id).addEventListener('change', function() {
                if (this.files.length > 0) {
                    ...
                    // Aktifkan kembali jika tidak ada file yang dipilih
                    inputs.forEach(otherId => {
